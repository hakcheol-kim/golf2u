...
import WebKit
import SwiftyJSON
class BoxBuyPayWeb: VariousViewController , WKUIDelegate, WKNavigationDelegate {
    ...
    private var m_sUrl : String = "\(Single.DE_WEBAPIPROTOCOL)://\(Single.DE_URLSERVER)/api/Purchase/startPurchase";
    ...
    override func viewDidLoad() {
        ...
        contentController.add(self, name: "iosr2BBPW")
...
extension BoxBuyPayWeb: WKScriptMessageHandler {
    func goResult(payseq : String = "0", Point : String = "0", isError : Bool = true){
        if !isError {
            m_Data["purchase_seq"] = payseq;
            SO.setUserInfoKey(key: "point_total", value: Point);
            m_tClickEvent?.ClickEvent(type: 10, data: m_Data)
            var fm_sPTitle = "구매 완료".localized;
            var fm_sPContents = "박스구매가 완료되었습니다.".localized;
            if m_Data["payment_method"] == "4" {
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "iosr2BBPW" {
            if let dictionary: [String: String] = message.body as? Dictionary {
                if dictionary["errorcode"] == "0"{
                    goResult(payseq: dictionary["purchase_seq"] ?? "0", Point : dictionary["point"] ?? "0", isError : false)
                }else{
