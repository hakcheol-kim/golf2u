//
//  ProductCommnetPop.swift
//  golf2u
//
//  Created by 이원영 on 2020/10/14.
...
class ProductCommnetPop: VariousViewController {
    private let SO = Single.getSO();
    private let JS = JsonC();
    
    public var closeHandler: ((String, String)->())?//클로저
   ...
   required init?(coder aDecoder: NSCoder) {
    ...
    override func viewDidLoad() {
        ...
        uiContentInput.placeholder = "상품 후기를 등록해보세요.".localized
...
extension ProductCommnetPop: YPImagePickerDelegate {
...
extension ProductCommnetPop:UITextFieldDelegate{
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        LoadingHUD.show()
        m_isAcc = true;
        JS.ProductCommentInsert(param: ["account_seq":m_sUserSeq, "product_seq":m_sPSeq, "my_product_seq":m_sMyPSeq, "contents":fm_sCon], imgs: imgs, callbackf: ProductCommentInsertCallback)
        self.view.endEditing(true)
    ...
    func ProductCommentInsertCallback(alldata: JSON)->Void {
        if alldata["errorcode"] != "0"{
            self.MessagePop(msg: alldata["errormessage"].string!, btntype : 2)
        }else{
            let fm_sPointType = alldata["data"]["point_type"].stringValue ;
            let fm_sPoint = alldata["data"]["point"].stringValue;
            let fm_sPoint_given = alldata["data"]["point_given"].stringValue;
            SO.setUserInfoKey(key: Single.DE_USERCLOVER, value: fm_sPoint )
            closeHandler?(fm_sPointType, fm_sPoint_given)
