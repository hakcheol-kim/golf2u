...
fileprivate extension CardsCollectionViewLayout {
  ...
  fileprivate func computeLayoutAttributesForItem(indexPath: IndexPath,
    ...
    attributes.center = CGPoint(x: contentCenterX + spacing * CGFloat(visibleIndex),y: midY)
    
    //정렬 할때 높이는 나란히 하기위해 midY 값을 위에 서 그대로 사용
    //attributes.center = CGPoint(x: contentCenterX + spacing * CGFloat(visibleIndex),y: midY + spacing * CGFloat(visibleIndex))
